...
document.addEventListener('click', (eve) => {
  ...
  if (columnIndex === lastSortedColumn) {
    ...
    // Якщо інший стовпець, сортуємо за зростанням
    sortDirection = 1;
  }
...
function sortTable(columnIndex) {
  ...
  allRows.sort((a, b) => {
    ...
    return isNumeric
      ? (parseFloat(aValue) - parseFloat(bValue)) * sortDirection
      : aValue.localeCompare(bValue) * sortDirection;
  });
