...
const TABLE_FIELDS = ['name', 'position', 'office', 'age', 'salary'];
const CITIES = [
  'Tokyo',
  'Singapore',
  'London',
...
table.addEventListener('click', (clickOnRowEvent) => {
  ...
  if (row && row.parentNode.tagName === 'TBODY') {
    ...
    if (currentRow !== -1 && currentRow !== row.rowIndex - 1) {
      const previousRow = table.querySelector(
        `tbody tr:nth-child(${currentRow + 1}`,
      );
...
saveButton.addEventListener('click', (buttonSubmitEvent) => {
  buttonSubmitEvent.preventDefault();

  if (validateForm(form)) {
    addDataFromForm(form, tableBody);
    clearForm(form, CITIES);
  }
});
...
function sortTable(tbody, columnIndex, isReversed) {
  ...
  const sortByRows = rows.sort((row1, row2) => {
  ...
  sortByRows.forEach((row) => tbody.appendChild(row));
}

function validateForm(currentForm) {
  const TOP_POSITION = 10;

  const RIGHT_POSITION = 10;
