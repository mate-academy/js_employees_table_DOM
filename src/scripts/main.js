...
const numberToDollarString = new Intl.NumberFormat('en-US', {
  style: 'currency',
  currency: 'USD',
});
...
function sortEmployees(header) {
  return [...employeeRows].sort((em1, em2) => {
    ...
    if (header === 4) {
      compareResult
        = extractNumberFromCurrency(em1.children[header].innerText)
        - extractNumberFromCurrency(em2.children[header].innerText);
    }
  ...
  tbody.insertAdjacentHTML(
    ...
    `
      <tr>
        ...
        <td>${employee.age}</td>
        <td>$${employee.salary.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</td>
      </tr>
    `,
  );
}
