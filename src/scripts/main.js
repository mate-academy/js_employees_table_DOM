...
table.addEventListener('dblclick', function (e) {
  ...
  function deleteInput() {
    ...
    if (!checkInput(cellContent, input)) {
      pushNotification(10, 10, 'Error', 'Невірне значення типу', 'error');
      input.focus();
...
function checkForm() {
  ...
  if (form.position.value.length < 4) {
    return {
      type: 'error',
      message: 'Позиція має містити не менше 4 символів',
    };
  }
  ...
  if (isNaN(salary)) {
    return {
      type: 'error',
      message: 'Введено неправильне значення зарплати',
    };
  }
  ...
  if (form.name.value.length < 4) {
    return {
      type: 'error',
      message: 'Значення імені містить менше 4 літер',
    };
  }
  ...
  if (age < 18 || age > 90) {
    return {
      type: 'error',
      message: 'Має бути більше 18 і менше 90',
    };
  }

  return {
    type: 'success',
    message: 'Додано нового співробітника!',
  };
}
